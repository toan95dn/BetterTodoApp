...
const createLoginPage = (() => {
  ...
  const loginContainerContent = `<div class='imageContainer'>
...
const bindEventWithLoginPage = (() => {
  ...
  const signUp = (() => {
    ...
    function disPlayOneContainerAndTurnOffRest(
      displayContainer,
      ...turnOffContainers
    ) {
      displayContainer.classList.add("active");
      turnOffContainers.forEach((container) =>
    ...
    confirmSignUpButton.addEventListener("click", (event) => {
      event.preventDefault();
      if (
        passwordConfirmInput.value !== passwordSignupInput.value ||
