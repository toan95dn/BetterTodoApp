...
class TaskView {
  ...
  constructor(title, dueDate, priority, isDone) {
    ...
    this.#checkBoxView = this.createView(
      "check_box_outline_blank",
      "material-icons"
    );
    this.setStatusView(isDone);

    this.#taskView.append(
      this.#checkBoxView,
      this.#titleView,
      this.#dueDateView,
      this.#editButtonView,
  ...
  createShowDetailTaskView(title, dueDate, projectName, detail, priority) {
    ...
    popUpBg.innerHTML = `<div class='popUpDetail'>
                <div id='closeDetailButton' class="material-icons">close</div>
                <p>
                    <h3>${title}</h3>
                    <div><span style='font-weight:bold'> Due Date: </span> <span>${dayjs(
                      dueDate
