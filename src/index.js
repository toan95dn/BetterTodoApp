...
const bindEventsWithApp = (() => {
  ...
  const addTaskOptionButton = document.querySelector(
    ".createTaskButtonContainer"
  );
  const addProjectOptionButton = document.querySelector(
  ...
  const inputProjectSelection = document.querySelector(
  ...
  createTaskForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const newTaskData = {
      title: inputTasksTitle.value,
      detail: inputDescription.value,
      dueDate: inputDueDate.value,
      priority: inputPriority.value,
      isDone: false,
      projectName: inputProjectSelection.value,
    };

    const firebaseID = await FireBaseManager.addTaskFirebase(newTaskData);
    const newTaskModel = new TaskModel(
      inputTasksTitle.value,
      inputDescription.value,
      inputDueDate.value,
      inputPriority.value,
      false,
      inputProjectSelection.value,
      firebaseID
    );
