...
const FireBaseManager = (() => {
  ...
  const loadUserData = async (user) => {
    //If the user is new, then set a default project called Inbox for the user
    const uid = user.uid;
    ...
    if (!docSnap.exists()) {
      ...
      const querySnapshot = await getDocs(
        collection(db, "users", uid, "AllTasks")
      );
      ...
      querySnapshot.forEach((doc) => {
        ...
        const currTaskModel = new TaskModel(
          currTaskData.title,
          currTaskData.detail,
          currTaskData.dueDate,
          currTaskData.priority,
          currTaskData.isDone,
          currTaskData.projectName,
          firebaseID
        );
  ...
  async function deleteProjectFireBase(projectName) {
    ...
    if (user) {
      ...
      if (uid !== "tUNpmmnXGdgMAUddvo97QXBathn2") {
        //This special ID is for Demo account, people can't touch the data of this account
        const projectNamesRef = doc(db, "users", uid);
        updateDoc(projectNamesRef, { ProjectNames: arrayRemove(projectName) });
        const queryAllTasksOfProject = query(
          collection(db, "users", uid, "AllTasks"),
  ...
  function addProjectFirebase(newProjectName) {
    ...
    if (user) {
      ...
      if (uid !== "tUNpmmnXGdgMAUddvo97QXBathn2") {
        //This special ID is for Demo account, people can't touch the data of this account
        const projectNamesRef = doc(db, "users", uid);
        updateDoc(projectNamesRef, {
          ProjectNames: arrayUnion(newProjectName),
        });
      }
    }
  }
